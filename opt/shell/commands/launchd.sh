...
kits_launchd_init() {
    kits_launchd_init_ "$KITS/etc/launchd.plist" "net.jeeker.kits.*.plist" "$HOME/Library/LaunchAgents"
}
...
kits_launchd_root_init() {
    kits_sudo_func kits_launchd_init_ "$KITS/etc/launchd.plist" "net.jeeker.root.kits.*.plist" "/Library/LaunchDaemons"
}
